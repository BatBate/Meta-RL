...
def gru(x, t, a, rew, rnn_state, gru_units, activation, output_size):
    inputs = tf.concat([x, tf.cast(t, tf.float32), a, rew], 2)  
...
def gru_categorical_policy(x, t, a, rew, rnn_state, gru_units, activation, output_size, act_dim):
    logits, state_out = gru(x, t, a, rew, rnn_state, gru_units, activation, output_size)
...
def gru_actor_critic(x, t, a, rew, pi_rnn_state, v_rnn_state, gru_units, act_dim, obs_dim, activation=tf.nn.relu):
    # only consider discrete experiment now
    policy = gru_categorical_policy
    x = tf.squeeze(tf.one_hot(x, depth=obs_dim), axis=2)
...
#                                                     activation, act_dim, 
#                                                     action_space)
        pi, logp, logp_pi, new_pi_rnn_state, prob = policy(x, t, a, rew, 
                                                     pi_rnn_state, gru_units, 
                                                     activation, act_dim, 
                                                     act_dim)
    with tf.variable_scope('v'):
        v, new_v_rnn_state = gru(x, t, a, rew, v_rnn_state, gru_units, activation, 1)
