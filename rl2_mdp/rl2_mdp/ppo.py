...
class PPOBuffer:
    ...
    def __init__(self, obs_dim, act_dim, size, gamma=0.99, lam=0.95):
        ...
        self.ter_buf = np.zeros(size, dtype=np.float32)
    ...
    def store(self, obs, ter, act, rew, val, logp):
        """
    ...
    def get(self):
        ...
        return [self.obs_buf, self.ter_buf, self.act_buf, self.rew_buf, 
...
#    x_ph = tf.placeholder(dtype=tf.int32, shape=(None, None, 2), name='x_ph')
    x_ph = tf.placeholder(dtype=tf.int32, shape=(None, None, 1), name='x_ph')
    t_ph = tf.placeholder(dtype=tf.int32, shape=(None, None, 1), name='t_ph')
...
#            action_space=env.action_space)
    pi, logp, logp_pi, v, pi_rnn_state, v_rnn_state, prob = actor_critic(
            x_ph, t_ph, a_ph, r_ph, pi_rnn_state_ph, v_rnn_state_ph, gru_units,
...
#        print(inputs[r_ph][:13])
        ...
        inputs[r_ph] = inputs[r_ph].reshape(-1, sequence_length, 1)
...
#                    print("Action probability distributation:", prob_t)
                ...
                t = ep_len == max_ep_len
                # save and log
                buf.store(o, int(t), last_a, last_r, v_t, logp_t)
...
if __name__ == '__main__':
    ...
    num_actions = 5
    env = TabularMDPEnv
    env_fn = env(num_states, num_actions)
