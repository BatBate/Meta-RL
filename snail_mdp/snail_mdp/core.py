...
MASK = np.array([[-float('inf') if i>j else 1 for i in range(SEQUENCE_LENGTH)] for j in range(SEQUENCE_LENGTH)])
...
def snail_bandit(x, t, a, r, seq_length, act_dim, obs_dim):
    x = tf.one_hot(x, depth=obs_dim)
    print("shape of x after one hot:", x.shape)
    a = tf.one_hot(a, depth=act_dim)
    ...
    inputs = tf.concat([x, tf.cast(t, tf.float32), a, r], 2)
    ...
    with tf.variable_scope('pi'):
        pi_num_filter = 32
        policy_net = tcb_lock(input_layer, seq_length, pi_num_filter)
        policy_net = tcb_lock(policy_net, seq_length, pi_num_filter)
        pi_key_size = 32
        pi_value_size = 32
    ...
    with tf.variable_scope('v'):
        v_num_filter = 16
        value_net = tcb_lock(input_layer, seq_length, v_num_filter)
        value_net = tcb_lock(value_net, seq_length, v_num_filter)
        v_key_size = 16
        v_value_size = 16
...
def snail_actor_critic(x, t, a, r, seq_length, act_dim, obs_dim):
    # only consider discrete experiment now
    policy = snail_bandit
    pi, logp, logp_pi, v = policy(x, t, a, r, seq_length, act_dim, obs_dim)
    return pi, logp, logp_pi, v